...
const scrap_bike = async (input, brand) => {
    try {
        ...
        if ('items' in data_res_arr) {
            ...
            let promiss = data_res_arr.items.map(async (val) => {
                let inc = 0
                // console.log(">>>>>>>>", val)
                brand_id = brand.id
                ...
                const bodyTypedata = await fetchOrCreateBodyType(category_id, style_type);

                // let [rowsd, files] = await con.query("SELECT * FROM `bodytypes` WHERE `category_id`=" + `'${category_id}'` + " AND `name` LIKE " + `'${style_type}'`)
                // let bodyTypedata = rowsd[0]

                // if (bodyTypedata) {
                //     bodytype_id = bodyTypedata?.id
                // } else {
                //     const qr = "INSERT INTO bodytypes(category_id,name,image,status,position )" + " VALUES " + `(${category_id},'${style_type}','',1,0)`;
                //     let craete = await con.query(qr);
                //     console.log("craete>>>>>>", craete)
                //     bodytype_id = craete[0]?.insertId;

                // }

                // Set a default value, or handle differently if needed.
                // const myPromise = new Promise(async (resolve, reject) => {
                //     let [row] = await con.query("SELECT * FROM `bodytypes` WHERE `category_id`=" + `'${category_id}'` + " AND `name` LIKE " + `'${style_type}'`)
                //     if (row) {
                //         console.log("row", row)
                //         bodytype_id = row[0]?.id
                //     } else {
                //         const qr = await con.query("INSERT INTO bodytypes(category_id,name,image,status,position )") + ' VALUES ' + (`(${category_id},'${style_type}','',1,0)`)
                //         console.log("qr", qr)
                //         bodytype_id = qr[0]?.insertId
                //     }
                // }).then(response => {
                //     return response
                // });
                ...
                if (bike_exist) {
                    ...
                    const qr = ("UPDATE " + `vehicle_information ` + "SET " + `brand_id = ${brand_id}, category_id = ${category_id}, bodytype_id = ${bodytype_id}, model_name = '${model_name}',fuel_type = '${fuel_type}',avg_rating = ${avg_rating}, review_count = ${review_count} ,variant_name = '${variant_name}',min_price=${min_price},max_price=${max_price},image='${image}',status='${status}', launched_at='${launched_at}',Launch_date='${Launch_date}',model_popularity=${model_popularity},mileage='${mileage}',engine=${engine},style_type='${style_type}',max_power='${max_power}',showroom_price=${showroom_price},rto_price=${rto_price ? parseInt(rto_price.replaceAll(',', '')) : 0},insurance_price=${insurance_price ? parseInt(insurance_price.replaceAll(',', '')) : 0},other_price=${other_price ? other_price.replaceAll(",", "") : 0} ,is_content_writer=${is_content_writer},on_road_price=${on_road_price},is_popular_search=${is_popular_search},is_upcoming=${is_upcoming},is_latest=${is_latest},link='${link}' WHERE brand_id = ${brand_id} AND model_name LIKE '${model_name}'`)
                    const update = await con.query(qr)
                    // console.log("bike_exist")
                    // console.log(">>>>>>>>>", await update)
                    // chneg by shrut
                    // let d = await get_other_details(model_url, images_url, brandobj, update[0].insertId)
                    // chneg by jignesh
                    let d = await get_other_details(model_url, images_url, brandobj, bike_exist.id)
                    // console.log("bike_exist", bike_exist)

                    // console.log("Update ____")
                } else {
                    // var res = await vehicle_information.create(brandobj)
                    const qr = ("INSERT INTO vehicle_information( brand_id, category_id, bodytype_id, model_name, fuel_type, avg_rating, review_count, variant_name, min_price, max_price, price_range, image, status, launched_at, Launch_date, model_popularity, mileage, engine, style_type, max_power, showroom_price, rto_price, insurance_price, other_price, is_content_writer, on_road_price, is_popular_search, is_upcoming, is_latest, link )") + ' VALUES ' + (`(${brand_id}, ${category_id},${bodytype_id},'${model_name}','${fuel_type}',${avg_rating},${review_count},'${variant_name}',${min_price},${max_price},'${price_range}','${image}','${status}','${launched_at}','${Launch_date}',${model_popularity},'${mileage}',${engine},'${style_type}','${max_power}',${showroom_price},${rto_price ? parseInt(rto_price.replaceAll(',', '')) : 0},${insurance_price ? parseInt(insurance_price.replaceAll(',', '')) : 0},${other_price ? other_price.replaceAll(",", "") : 0},${is_content_writer},${on_road_price},${is_popular_search},${is_upcoming},${is_latest},'${link}')`)
                    let craete = await con.query(qr)
                    console.log("bike_exist craete")
                    await get_other_details(model_url, images_url, brandobj, craete[0].insertId)

                }
            })
            async function fetchOrCreateBodyType(category_id, style_type) {
                let [rowsd, files] = await con.query("SELECT * FROM `bodytypes` WHERE `category_id`=" + `'${category_id}'` + " AND `name` LIKE " + `'${style_type}'`);
                let bodyTypedata = rowsd[0];

                if (!bodyTypedata) {
                    const qr =
                        "INSERT INTO bodytypes(category_id,name,image,status,position )" +
                        " VALUES " + `(${category_id},'${style_type}','',1,0)`;

                    let craete = await con.query(qr);
                    console.log("craete>>>>>>", craete);
                    bodyTypedata = { id: craete[0]?.insertId };
                }
                ...
                axios.get("https://www.bikedekho.com" + val.otherLinks.url).then(async (client) => {
                    ...
                    if (html) {
                        ...
                        if (bike_exist) {
                            ...
                            const update = await con.query(qr)
                            await get_other_details(model_url, images_url, dataObj, bike_exist.id)
                            ...
                            // var res = await vehicle_information.create(dataObj)
                            let craete = await con.query(qr)
                            await get_other_details(model_url, images_url, dataObj, craete[0].insertId)
...
const get_other_details = async (url, images_url, dataObj, model_id) => {
    ...
    await axios.get(url).then(async (res) => {
        ...
        if ('colors' in colors_data_arr) {
            var colors = colors_data_arr.colors.map(async (val) => {
                ...
                // var exist = await vehicle_model_color.find({ $and: [{ vehicle_information_id: model_id }, { color_code: val.hexCode }] }).count()
                ...
                if (bike_exist) {
                    ...
                    const qr = ("UPDATE " + `vehicle_model_color ` + "SET " + `vehicle_information_id = ${model_id}, color_name = '${val.name}', color_code = '${val.hexCode}', image = '${image}' WHERE vehicle_information_id = ${model_id} AND color_code = '${val.hexCode}'`)
                    const update = await con.query(qr)
                } else {
                    // res = await vehicle_model_color.create(input_color)
                    const qr = ("INSERT INTO vehicle_model_color ( vehicle_information_id, color_name, color_code, image)") + ' VALUES ' + (`(${model_id}, '${val.name}','${val.hexCode}','${image}')`)
                    let craete = await (await con.query(qr))
    ...
    await axios.get(url).then(async (responsed) => {
        ...
        $(responsed.data).find('table[class="allvariant contentHold"] tbody tr').each(async (index, node) => {
            ...
            // var exist = await PriceVariant.findOne({ $and: [{ vehicle_information_id: model_id }, { name: model_name }] })
            const [rows, filed] = await con.query("SELECT * FROM `vehicle_price_variant` WHERE `vehicle_information_id`= " + `${model_id}` + " AND `name` = " + `'${model_name}'`)
            const exist = rows[0]

            if (exist) {
                // await PriceVariant.findOneAndUpdate({ $and: [{ vehicle_information_id: model_id }, { name: model_name }] }, dataobje, { new: true })
                const qr = ("UPDATE " + `vehicle_price_variant ` + "SET " + `vehicle_information_id = ${vehicle_information_id},launched_at='${launched_at}',rating=${rating},review_count=${review_count},other_price=${other_price},rto_price=${rto_price},insurance_price=${insurance_price},on_road_price=${on_road_price ? on_road_price.replaceAll(",", "") : 0}, mileage=${mileage},ex_show_room_rice=${ex_show_room_rice},fuel_type = '${fuel_type}', name = '${name}',status='${status}', link = '${link}', price_range= '${price_range}', engine = '${engine}' WHERE vehicle_information_id = ${model_id} AND name = '${model_name}'`)
                const update = await con.query(qr)

            } else {
                // var sub_model = await PriceVariant.create(dataobje)

                const qr = ("INSERT INTO vehicle_price_variant( vehicle_information_id, name, link, engine, status, fuel_type, ex_show_room_rice, price, mileage, on_road_price, price_range, insurance_price, rto_price, other_price, review_count, rating, launched_at )") + ' VALUES ' + (`(${vehicle_information_id}, '${name}','${link}','${engine}','${status}','${fuel_type}',${ex_show_room_rice},'${price}',${Number(mileage)},${on_road_price ? on_road_price.replaceAll(",", "") : 0},'${price_range}','${insurance_price}','${rto_price}','${other_price}',${review_count},${rating},'${launched_at}')`)
                let craete = await con.query(qr)
                await get_bike_specification(link, model_id, craete[0].insertId, dataobje)
...
const get_bike_specification = async (url, vehicle_information, variant = 0, dataobje) => {
    var used_var
    var colors_data = await scrap_common_model(url)
    ...
    if ('specsTechnicalJson' in colors_data) {
        if ('specification' in colors_data.specsTechnicalJson) {
            colors_data.specsTechnicalJson.specification.map(async (value) => {
                ...
                if (spec_exist) {
                    ...
                    // spec_id = await VariantSpecification.create(varobj)
                ...
                value.items.map(async (values) => {
                    ...
                    // console.log(" vehicle_information_id variant_id spec_id spec_name", vehicle_information_id, "+", variant_id, "+", spec_id, "+", spec_name)
                    // console.log([vehicle_information_id, variant_id, spec_id, spec_name])

                    let qr = `SELECT * FROM variant_key_specs WHERE vehicle_information_id = ? AND variant_id = ? AND specification_id = ? AND name LIKE ?`;
                    let [rows, filed] = await con.query(qr, [vehicle_information_id, variant_id, spec_id, `${spec_name}`])
        ...
        if ('keySpecs' in colors_data.specsTechnicalJson) {
            colors_data.specsTechnicalJson.keySpecs.map((valudata) => {
                // console.log("valudata>>>>", valudata)
                if (valudata.title.toLowerCase().includes("specifications")) {
                    var is_specification = 1
                    var i = valudata.items.map(async (valdatas) => {
                        let [rows, filed] = await con.query("SELECT * FROM `variant_key_specs` WHERE `vehicle_information_id`= " + `${vehicle_information_id} AND variant_id = ` + `${variant_id}  AND name = '${valdatas.text}'`)
                        const u = rows[0]
