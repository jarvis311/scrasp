...
async function getNextUniqueVariantKeyId() {
...
const get_bike_specification = async (url, vehicle_information, priceVariant = 0, dataobje,) => {
    ...
    async function processSpecifications(specs) {
        for (const spec of specs) {
            ...
            const spec_name = spec.title ? spec.title : "NA";
            const variantSpecification = await getOrCreateVariantSpecification(spec_name, idOfVarSpec);
            ...
            for (const item of spec.items) {
                ...
                const spec_name = item.text ? item.text : "NA";
                const spec_value = item.value ? item.value : "NA";
                const { show_overview, variant_key_id } = getShowOverviewAndVariantKey(spec_name);

                await createOrUpdateVariantKey(
                    vehicle_information_id,
                    variant_id,
                    variantSpecification._id,
                    spec_name,
                    spec_value,
                    show_overview,
                    variant_key_id,
    ...
    async function getOrCreateVariantSpecification(name, php_id) {
        let variantSpecification = await VariantSpecification.findOne({ name });
        if (!variantSpecification) {
            variantSpecification = await VariantSpecification.findOneAndUpdate({ name: name }, { name: name, php_id: php_id }, { upsert: true, new: true });
        }
        return variantSpecification;
    }

    function getShowOverviewAndVariantKey(spec_name) {
        const mapping = {
            'Motor Power': { show_overview: 1, variant_key_id: 32 },
            'Motor Type': { show_overview: 1, variant_key_id: 34 },
            'Battery Capacity': { show_overview: 1, variant_key_id: 33 },
            // Add more mappings as needed
        };

        return mapping[spec_name] || { show_overview: 0, variant_key_id: 0 };
    }

    async function createOrUpdateVariantKey(
        vehicle_information_id,
        variant_id,
        specification_id,
        name,
        ...
        const existingVariantKey = await VariantKey.findOneAndUpdate(
            {
                vehicle_information_id,
                variant_id,
                specification_id,
    ...
    if ('specsTechnicalJson' in colors_data) {
        if ('specification' in colors_data.specsTechnicalJson) {
            ...
            //         const tokenIdOfVariantKey = cheakidOfVariantKey ? cheakidOfVariantKey.php_id + 1 : 1
            //         const VariantKeyIdUnique = tokenIdOfVariantKey
            //         // console.log('tokenIdOfVariantKey', value, "---------", values, "+++++", tokenIdOfVariantKey)
            ...
            //     }
            // }
            processSpecifications(colors_data.specsTechnicalJson.specification);
        }
